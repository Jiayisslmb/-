    ...
    const fetchPostDetails = async () => {
      try {
        ...
        setPost({
          ...
          mediaCid: postData.mediaCid || postData.coverCid,
          likes: postData._count?.articlelike || postData._count?.momentlike || postData.likes || 0,
          comments: postData._count?.articlecomment || postData._count?.momentcomment || postData.comments || 0,
          shares: postData._count?.articlerepost || postData._count?.momentrepost || postData.shares || 0,
        ...
        if (commentsRes.ok) {
          ...
          const replyKey = postType === 'article' ? 'articlecomment' : 'momentcomment';
  ...
  const handleReplySubmit = async (parentId: number, parentType: 'comment' | 'reply' = 'comment') => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const commentKey = post?.type === 'article' ? 'articlecomment' : 'momentcomment';
        const mappedReply = {
        ...
        if (parentType === 'comment') {
          ...
          // 找到包含被回复回复的评论，然后将新回复添加到该评论的replies数组中
          setComments(comments.map(comment => {
            if (comment.replies?.some(reply => reply.id === parentId)) {
              return { ...comment, replies: [...(comment.replies || []), mappedReply] };
            }
            return comment;
          }));
        }
              ...
              <div className="flex items-center flex-wrap gap-1 mt-0.5">
                ...
                {isArticle && post.circleId && post.circleName && (
                  <>
                    ...
                    <LinkWithBack href={`/circles/${post.circleId}`} className="text-[#6364FF] hover:underline text-sm font-medium">📍 {post.circleName}</LinkWithBack>
                  </>
                )}
                ...
                {isArticle && <span className="text-[#6364FF] text-xs font-medium bg-[#6364FF]/5 px-2 py-0.5 rounded-full ml-1">📝 文章</span>}
                {isMoment && <span className="text-green-500 text-xs font-medium bg-green-50 px-2 py-0.5 rounded-full ml-1">💬 动态</span>}
              </div>
            </div>
          </div>

          {isArticle && post.title && (
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight leading-snug">{post.title}</h1>
          )}

          {(post.content.includes('转发动态') || post.content.includes('转发文章')) ? (
            <div className="mb-4">
              {post.content.includes('原文链接:') && post.content.split('转发文章')[0]!.split('转发动态')[0]!.trim() && (
              ...
              <div className="p-4 bg-white border border-gray-100 rounded-lg">
                ...
                {post.content.includes('原文链接:') ? (
                  <>
                    <div className="text-gray-700 whitespace-pre-wrap leading-relaxed text-base mb-2">
                      {post.content.split('\n原文链接:')[0]!.replace('转发文章\n「', '').replace('」', '').replace('转发动态\n「', '').replace('」', '').replace(post.content.split('转发文章')[0]!.split('转发动态')[0]!, '').trim()}
                    </div>
                    <div className="mt-2">
                      <a href={post.content.match(/原文链接: (.*)/)?.[1] || ''} className="text-blue-600 hover:underline inline-flex items-center gap-1" target="_blank" rel="noopener noreferrer">
          ...
          <div className="flex gap-1 pt-4 border-t border-gray-100">
            ...
            {!(post.content.includes('转发动态') || post.content.includes('转发文章')) && (
              <button onClick={handleShare}
                ...
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.16m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" /></svg>
